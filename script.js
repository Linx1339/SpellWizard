...
function renderGameBoard() {
    ...
    for (let row = 0; row < 7; row++) {
        for (let col = 0; col < 7; col++) {
            ...
            cell.onclick = () => handleLetterClick(row, col);
...
function greyOutInvalidLetters() {
    const language = localStorage.getItem('selectedLanguage') || 'EN';
    const dictionary = JSON.parse(localStorage.getItem(language === 'SE' ? 'ordbok' : 'dictionary'));
...
// Function to load all ordbok files and combine their data
function loadOrdbokFiles() {
    const ordbokDir = 'ordbok'; // Directory containing the ordbok files
    const ordbokFiles = [
        'ordbok-A.js', 'ordbok-B.js', 'ordbok-C.js', 'ordbok-D.js', 'ordbok-E.js',
        // Add all other ordbok files here
    ];

    const promises = ordbokFiles.map(file => {
        return fetch(`${ordbokDir}/${file}`)
            .then(response => response.text())
            .then(scriptContent => {
                // Evaluate the script content to load the ordbok array
                eval(scriptContent);
                // Extract the ordbok array from the evaluated script
                const ordbokArrayName = file.replace('.js', '');
                return window[ordbokArrayName];
            });
    });

    return Promise.all(promises).then(ordbokArrays => {
        // Combine all ordbok arrays into one
        return ordbokArrays.flat();
    });
}
...
// Load the combined ordbok data and store it in IndexedDB
function loadOrdbok() {
    if (!localStorage.getItem('ordbokLoaded')) {
        loadOrdbokFiles().then(ordbok => {
            const dbRequest = indexedDB.open('OrdbokDB', 1);

            dbRequest.onupgradeneeded = function(event) {
                const db = event.target.result;
                const objectStore = db.createObjectStore('ordbok', { keyPath: 'id', autoIncrement: true });
                objectStore.createIndex('word', 'word', { unique: false });
            };

            dbRequest.onsuccess = function(event) {
                const db = event.target.result;
                const transaction = db.transaction(['ordbok'], 'readwrite');
                const objectStore = transaction.objectStore('ordbok');
                ordbok.forEach(word => {
                    objectStore.add({ word });
                });
                localStorage.setItem('ordbokLoaded', 'true');
            };
        }).catch(error => console.error('Error loading ordbok files:', error));
    }
}
